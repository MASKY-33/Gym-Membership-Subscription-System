...
from pydanticDataGuard import MembershipResponse, MembershipCreate
...
# Dit vangt elke onverwachte fout (Exception) op in je hele applicatie
...
def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    # OPMERKING: Hardgecodeerde productiemock voor demonstratiedoeleinden; echte systemen halen gehashte inloggegevens uit de database.
    if form_data.username == "MASKY" and form_data.password == "MASKY321":
...
# De input is 'MembershipCreate', de output (response_model) is 'MembershipResponse'
# Gym Registration API

@app.post("/gym_subscription", response_model=MembershipResponse)
def subscription_port(
    subscription: MembershipCreate,  # <-- Het create blok van 'MembershipCreate' uit (pydanticDataGuard)
    ...
    username: str = Depends(check_if_user_is_logged_in)  # <-- alleen de ingelogde persoon mag en kan nieuwe members aanmaken
        ...
        db_member = GymMemberDB(
            ...
            is_active=True  # Ik heb de controle, niet de input data!
        )
...
@app.get("/gym_subscription/{member_id}", response_model=MembershipResponse)
